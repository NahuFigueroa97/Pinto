...
export default function RecoverPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Dentro de la app de Android el origin es "https://localhost" (lo fija
      // Capacitor con androidScheme), así que el link del mail apuntaba a un
      // host inexistente y la recuperación de contraseña quedaba rota en el
      // celular. Hay que usar sí o sí la URL pública del sitio.
      const siteUrl = process.env.NEXT_PUBLIC_SITE_URL;
      if (!siteUrl) {
        throw new Error(
          'La app no está configurada para recuperar contraseñas. Escribinos a soporte@pinto.app.',
        );
      }

      const { error } = await supabase.auth.resetPasswordForEmail(email, {
        redirectTo: `${siteUrl.replace(/\/$/, '')}/actualizar-clave/`,
      });
