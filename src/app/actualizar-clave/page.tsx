...
export default function UpdatePasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Antes había un Promise.race contra un setTimeout de 5 s que resolvía
      // { error: null }: si la llamada tardaba más, la pantalla decía
      // "¡Clave actualizada!" aunque la contraseña NO se hubiera cambiado.
      // El usuario quedaba afuera de su cuenta creyendo que todo salió bien.
      const { error: updateError } = await supabase.auth.updateUser({ password });
      if (updateError) throw updateError;

      setSuccess(true);
    } catch (err: any) {
      setError(err?.message || 'No se pudo actualizar la contraseña. Probá de nuevo.');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!ready) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-surface fixed inset-0 z-[100]">
        <div className="text-center">
          <Loader2 className="w-10 h-10 text-brand-500 animate-spin mx-auto mb-4" />
          <p className="text-muted">Verificando link de seguridad...</p>
        </div>
      </div>
    );
  }
  ...
  if (success) {
    return (
      <div className="min-h-screen flex flex-col justify-center px-6 bg-surface fixed inset-0 z-[100]">
        <div className="max-w-sm mx-auto w-full text-center">
          <div className="w-16 h-16 bg-green-50 rounded-full flex items-center justify-center mx-auto mb-6">
            <CheckCircle2 className="text-green-500" size={28} />
          </div>
          <h1 className="text-2xl font-bold text-ink mb-2">¡Clave actualizada!</h1>
          <p className="text-muted mb-2">Tu contraseña fue cambiada con éxito.</p>
    ...
    <div className="min-h-screen flex flex-col justify-center px-6 bg-surface fixed inset-0 z-[100]">
      <div className="max-w-sm mx-auto w-full">
        <div className="text-center mb-8">
          ...
          <p className="text-muted mt-2">Elegí una clave que no hayas usado antes</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-ink-soft mb-1">Nueva contraseña</label>
          ...
          <div>
            <label className="block text-sm font-medium text-ink-soft mb-1">Confirmar contraseña</label>
            <input
              type={showPassword ? 'text' : 'password'}
