...
function NegocioMensajesInner() {
  ...
  if (!business) return (
    <div className="max-w-lg mx-auto flex flex-col items-center justify-center min-h-[60vh] text-center px-6">
      ...
      <p className="text-sm text-faint mb-6">Cuando lo crees vas a poder recibir consultas de clientes acá</p>
          ...
          {chatMessages?.map((msg: any) => (
            <div key={msg.id} className={`flex ${msg.sender_role === 'business' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[75%] px-3.5 py-2 rounded-2xl text-sm ${
                ...
                <p>{msg.message}</p>
                <p className={`text-[0.5rem] mt-0.5 ${msg.sender_role === 'business' ? 'text-accent-200' : 'text-faint'}`}>
        ...
        <div className="px-4 py-3 border-t border-line bg-surface">
          <div className="flex items-center gap-1">
            ...
            <input
              ...
              className="flex-1 px-4 py-2.5 rounded-xl border border-line-strong text-sm focus:border-accent-400 outline-none"
            />
  ...
  return (
    <div className="max-w-lg mx-auto pb-20">
      <header className="px-4 pt-6 pb-3">
        ...
        <p className="text-sm text-faint">Consultas de clientes sobre tus promos</p>
      </header>

      {!conversations?.length ? (
        <div className="text-center py-16 px-4">
          <p className="text-5xl mb-3">📭</p>
          <p className="text-muted font-medium">Sin mensajes aún</p>
          <p className="text-sm text-faint mt-1">Cuando un usuario te consulte por una promo, vas a verlo acá</p>
        </div>
      ) : (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  ...
                  <p className="text-[0.55rem] text-faint">{new Date(conv.created_at).toLocaleDateString('es-AR', { day: 'numeric', month: 'short' })}</p>
                </div>
