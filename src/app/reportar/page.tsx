...
function ReportarInner() {
  ...
  if (success) return (
    <div className="max-w-lg mx-auto flex flex-col items-center justify-center min-h-[60vh] text-center px-6">
      ...
      <p className="text-muted text-sm mb-6">Vamos a revisarlo lo antes posible. Gracias por cuidar la comunidad 💛</p>
    ...
    <div className="max-w-lg mx-auto pb-8">
      <header className="flex items-center gap-3 px-4 pt-6 pb-4">
        ...
        <div>
          ...
          <p className="text-xs text-muted">Ayudanos a mantener Pintó seguro</p>
        </div>
      </header>

      <div className="px-4 space-y-4">
        <p className="text-sm text-muted">¿Por qué querés reportar?</p>
        <div className="space-y-2">
          {REASONS.map(r => (
            <button key={r.value} onClick={() => setReason(r.value)}
              className={`w-full text-left p-3 rounded-xl border text-sm font-medium transition ${
                reason === r.value ? 'border-brand-400 bg-brand-50 text-brand-700' : 'border-line-strong text-ink-soft'
              }`}>
              {r.label}
            </button>
          ))}
        ...
        <textarea
          ...
          className="w-full px-4 py-3 rounded-xl border border-line-strong text-sm outline-none focus:border-brand-400"
          rows={3}
        />

        {report.isError && (
          <div className="bg-red-50 text-red-600 dark:bg-red-500/15 dark:text-red-300 text-sm px-4 py-3 rounded-xl">
